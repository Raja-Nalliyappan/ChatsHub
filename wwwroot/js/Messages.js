...
function renderMessage(container, messageObj) {
    ...
    menuBtn.style.color = isCurrentUser ? "#fff" : "#000";
    ...
    menu.innerHTML = `
        <div class="msg-option" data-action="copy">📋 Copy</div>
        ${isCurrentUser ? `<div class="msg-option" data-action="delete">🗑 Delete</div>` : ""}
    `;
    ...
    menu.addEventListener("click", async (e) => {
        ...
        if (action === "delete") {
            ...
            try {
                ...
                const receiverId = SELECTED_USER_ID;

                const response = await fetchWithAuth(
                    `/ChatsHub/Chat/DeleteChatMessage?receiverId=${receiverId}&id=${messageId}`,
                    { method: "DELETE" }
                );
